...
// 箇条書き内画像サイズ（テンプレート5専用）
export type BulletImageSize = 'S' | 'M' | 'B';

// 箇条書き内画像（テンプレート5専用）
export interface BulletImage {
  src: string;           // base64データ（data:プレフィックス付き）
  mimeType: string;      // image/png, image/jpeg 等
  size: BulletImageSize; // S=1行分, M=2行分, B=3行分
...
// スライド表示モード（箇条書き or 本文）
export type SlideDisplayMode = 'bullets' | 'body';
...
export interface Slide {
  ...
  bullets: string[];          // 箇条書き（最大5行）または本文行
  bulletLevels?: number[];    // 各箇条書きの見出しレベル（1/2/3）- 未指定時は2
  bulletImages?: (BulletImage | null)[];  // bullets[]と並列。null=テキスト行、BulletImage=画像行（base5専用）
  bodyHtml?: string;          // 本文リッチテキスト（HTML）- 未設定時はbulletsをプレーン表示
  speakerNotes: string;       // スピーカーノート
  layoutType: SlideLayoutType;
  // === 表示モード ===
  displayMode?: SlideDisplayMode;  // 'bullets'(箇条書き) or 'body'(本文) - 未指定時はbullets
  // === カラム数（base5フリーレイアウト用）===
  columnCount?: 1 | 2 | 3;         // 箇条書きのカラム数 - 未指定時は1
  // === ベーステンプレート（レイアウト切替）===
  templateId?: BaseTemplateId; // ベーステンプレートID（未指定時はbase1）
  // === 画像表示モード ===
  imageDisplayMode?: 'cover' | 'contain';  // 画像の表示モード（未指定時はテンプレートデフォルト: accent=contain, full=cover）
  // === Visual（新構造）===
...
export interface ScriptGenerationResult {
  ...
  slideBullets: string[];   // スライド用箇条書き（最大5つ）
  speakerNotes: string;     // スピーカーノート
}
