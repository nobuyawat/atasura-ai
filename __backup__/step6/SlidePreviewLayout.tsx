...
function TextContent({
  ...
  // 箇条書きの有無を判定
  const hasBullets = slide.bullets && slide.bullets.length > 0 && slide.bullets.some(b => b.trim());
  const hasBodyText = bodyText && bodyText.trim();
  ...
  return (
    ...
    >
      ...
      {hasBullets && (
        isTwoColumn ? (
          <div className={`grid grid-cols-2 gap-3 overflow-y-auto ${hasBodyText ? '' : 'flex-1'}`}>
      ...
      {hasBodyText && (
        <div className={`${hasBullets ? 'mt-2 pt-2 border-t border-slate-100' : 'flex-1'} overflow-y-auto`}>
          <p className={`${hasBullets ? 'text-[9px] text-slate-500' : 'text-[11px] text-slate-600'} leading-relaxed`}>
            {bodyText}
          </p>
        </div>
      )}
      ...
      {!hasBullets && !hasBodyText && (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-[10px] text-slate-400">中央エディタで内容を入力してください</p>
        </div>
      )}
...
export function SlidePreviewLayout(props: SlidePreviewLayoutProps) {
  ...
  // =====================================================
  // リアルタイムプレビュー用データ構築
  // livePreviewが渡されている場合はそれを優先、なければslideのデータを使用
  // =====================================================
  const displayTitle = livePreview?.title || slide.title || 'タイトル未設定';
  const displayBullets = (livePreview?.bullets && livePreview.bullets.length > 0)
    ? livePreview.bullets
    : slide.bullets;
  const displayBodyText = livePreview?.bodyText || bodyText || '';

  // 表示用のスライドオブジェクト（テキスト関連のみ上書き）
  const displaySlide = {
    ...slide,
    title: displayTitle,
